...
const POIS_PATH = path.join(ROOT, "public", "data", "cities", "oxford", "pois.json");

const slim = (rowsFromSlimPayload(JSON.parse(readFileSync(SLIM_PATH, "utf8"))) ?? []) as SlimVenue[];
const slimIds = new Set(slim.map((v) => v.id));

describe("Oxford Freshers editorial pack", () => {
  const landmarks = landmarksForCity("oxford");
  const bands = storyBandsForCity("oxford");
  const crawls = curatedCrawlsForCity("oxford");
  ...
  it("ships ≥8 sourced landmarks", () => {
    ...
    for (const landmark of landmarks) {
      ...
      expect(lat).toBeGreaterThan(51.72);
      expect(lat).toBeLessThan(51.8);
      expect(lng).toBeGreaterThan(-1.3);
      expect(lng).toBeLessThan(-1.2);
    }
  });

  it("ships ≥3 valid story bands including freshers-first-night for deep links", () => {
    expect(bands.length).toBeGreaterThanOrEqual(3);
    expect(bands.some((b) => b.id === "freshers-first-night")).toBe(true);
    expect(validateAllStoryBands(bands, landmarks)).toEqual({});
  });

  it("ships ≥2 curated crawls whose stop ids exist in the Oxford slim index", () => {
  ...
  it("keeps London selectors on the London catalogs", () => {
    ...
    expect(landmarksForCity("oxford").some((l) => l.id === "tower-bridge")).toBe(false);
    expect(storyBandsForCity("oxford").some((b) => b.id === "river-history")).toBe(false);
  });

  it("points CityConfig.poisPath at a valid Oxford POI seed", () => {
    expect(CITIES.oxford.poisPath).toBe("/data/cities/oxford/pois.json");
